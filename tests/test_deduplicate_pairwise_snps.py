...
import pandas as pd
from evaluate.deduplicate_pairwise_snps import DeduplicatePairwiseSNPsUtils, Allele, PairwiseVariation, \
    DeduplicationGraph, PangenomeVariation, PangenomeVariations, ConsistentPangenomeVariations, \
    InconsistentPangenomeVariations, \
...
class TestConsistentPangenomeVariations(TestCase):
    ...
    @patch.object(DeduplicatePairwiseSNPsUtils,
                  DeduplicatePairwiseSNPsUtils._get_ref_and_query_from_ShowSNPsDataframe_filepath.__name__,
...
class TestDeduplicationGraph(TestCase):
    ...
    def test____index_pairwise_variation___several_variations(self, *mocks):
        ...
        expected_indexing = {self.alleles[0]: {pairwise_variations[0], pairwise_variations[1], pairwise_variations[2],
                                               pairwise_variations[4]},
                             self.alleles[1]: {pairwise_variations[0], pairwise_variations[2], pairwise_variations[3],
                                               pairwise_variations[5]},
                             self.alleles[2]: {pairwise_variations[1], pairwise_variations[3], pairwise_variations[4],
                                               pairwise_variations[5]},
    ...
    def test___add_variants_from_ShowSNPsDataframe_core___one_variation_in_df___one_variation_added(self):
        ...
        assert self.deduplication_graph.graph.number_of_nodes() == 1 and self.deduplication_graph.graph.has_node(
            pairwise_variation)

    def test___add_variants_from_ShowSNPsDataframe_core___one_SNP_variation_four_non_SNP_variation_in_df___only_SNP_variation_added(
            self):
        snps_df = pd.read_csv(StringIO(
            """ref_pos,ref_sub,query_sub,query_pos,nearest_mismatch,nearest_end,ref_len,query_len,ref_context,query_context,ref_strand,query_strand,ref_chrom,query_chrom
    ...
    def test___add_variants_from_ShowSNPsDataframe_core___three_SNP_variation_four_non_SNP_variation_in_df___only_SNPs_variations_added(
            self):
        snps_df = pd.read_csv(StringIO(
            """ref_pos,ref_sub,query_sub,query_pos,nearest_mismatch,nearest_end,ref_len,query_len,ref_context,query_context,ref_strand,query_strand,ref_chrom,query_chrom
